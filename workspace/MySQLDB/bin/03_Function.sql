...
-- FLOOR(숫자) - 값보다 작은 정수 중 가장 큰수 [실수를 무조건 버림(음수일 경우는 제외)]
...
-- DATE_SUB( 날짜, INTERVAL 뺄숫자 기준값 ) - 날짜에서 기준값단위로 뺄숫자만큼 뺀다.
...
SELECT CONCAT(MONTH(now()), '월') AS 월 from dual;
...
SELECT str_to_date('20211013', '%Y%m%d') from dual; --20211013
