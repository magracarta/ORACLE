...
-- 		varchar2(10) 에 "abcd"를 저장하면 4글자만큼의 용량만 소비된다.

-- 4. 고정형 문자열(최대 2000Byte)
--		char(XX)
--		char(10) 에 "abcd"를 저장하면 "abcd      "가 저장 10글자 만큼의 용량을 유지.
...
-- CLOB, NCLOB, BLOB등등이 있지만, 요새는 대용량 파일은 별도의 서버에 저장하고 경로와 이름을 데이터베이스로
-- 관리하는 형식을 많이 사용한다.
